...
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'activity_interactions') THEN
    -- Verificar si las columnas de embedding ya existen
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'activity_interactions' AND column_name = 'user_embedding') THEN
      -- Añadir columnas de embeddings vectoriales a la tabla existente
      ALTER TABLE activity_interactions ADD COLUMN user_embedding vector(1536);
      ALTER TABLE activity_interactions ADD COLUMN ai_embedding vector(1536);
      ...
      RAISE NOTICE 'Columnas de embeddings añadidas correctamente a la tabla activity_interactions';
    ELSE
      RAISE NOTICE 'Las columnas de embeddings ya existen en la tabla activity_interactions';
    END IF;
  ELSE
    -- Si la tabla no existe, crearla con soporte vectorial
    ...
    RAISE NOTICE 'Tabla activity_interactions creada correctamente con soporte vectorial';
  END IF;
END $$;

-- Crear o reemplazar la función para buscar mensajes semánticamente similares
...
BEGIN
  ...
  WHERE
    ...
    ai.user_embedding IS NOT NULL AND
